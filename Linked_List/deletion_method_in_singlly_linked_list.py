...
class SLinkedList:
    ...
    def deleteNode(self, location ):
        ...
        # Check if the list exists (not empty)
        if self.head is None:
            print("The SLL does not exist")
        else:
            # Case 1: Delete the first node (head)
            if location == 0:
                if self.head == self.tail:  # Only one node in the list
                    self.head = None         # Remove head
                    self.tail = None        # Remove tail
                else:
            ...
            # Case 3: Delete a node from a specific position (middle)
            else:
                tempNode = self.head
                index = 0
                ...
                nextNode = tempNode.next            # Node to delete
                tempNode.next = nextNode.next       # Skip the node (delete it)
